...
int main()
{
	...
	do
	{
		...
		displayBoard(gameBoard);
		//If the game is not over, let player 2 have a turn.
...
// The displayBoard function displays the contents of the board
...
//Creates game board
void createBoard(string board[][COLS])
{
...
// The playerTurn function allows a player (X or O) to take a turn.
void playerTurn(string board[][COLS], string symbol)
{
	/* The isAvailable flag is set to true when the player selects a location on the board that
	is available. (initialized to false until set to true when player selects a location on the board that is available) */
...
// The gameOver function returns true if the game is over. This is the case when either player has already won, or there is a tie.
...
// The playerWins function accepts the game board and a player symbol (X or O) as arguments. It returns true if the player has won.
...
// The playerCanWin function returns true if the player (X or O) can still win.
bool playerCanWin(string board[][COLS], string symbol)
{
	...
	// Check the other diagonal for a possibility.
	if ((board[2][0] == symbol || board[2][0] == "*") &&
		(board[1][1] == symbol || board[1][1] == "*") &&
		(board[0][2] == symbol || board[0][2] == "*"))
		status = true;
